#!/usr/bin/env python3
"""Train Model C-Graph: Model B plus two graph relationship features (card1-DeviceInfo and addr1-card1 historical associations)."""
...
from graph_relationship_features import (
    ...
    graph_rel_feature_specification,
)
...
FEATURES = model_b.FEATURES + GRAPH_REL_FEATURES
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/models/model_c_graph"))
    parser.add_argument("--report", type=Path, default=Path("docs/MODEL_C_GRAPH_BASELINE.md"))
...
def attach_graph_rel_features(train: pl.DataFrame, validation: pl.DataFrame) -> tuple[pl.DataFrame, pl.DataFrame, dict[str, Any]]:
    """Build state over train then validation events; labels never enter this function."""
    # Select the columns needed for graph relationship: TransactionID, TransactionDT, card1, DeviceInfo, addr1
    rel_train = train.select(*GRAPH_REL_INPUT_COLUMNS)
    rel_validation = validation.select(*GRAPH_REL_INPUT_COLUMNS)
    rel_events = pl.concat([rel_train, rel_validation], how="vertical")
    rel_features = build_graph_rel_features(prepare_graph_rel_events(rel_events))
    evidence = graph_rel_feature_specification() | {
        "rel_event_row_count": rel_events.height,
        "train_rel_event_row_count": rel_train.height,
        "validation_rel_event_row_count": rel_validation.height,
        "label_column_passed_to_graph_builder": False,
        "same_timestamp_batching": True,
    }
    return (
        train.join(rel_features, on="TransactionID", how="left"),
        validation.join(rel_features, on="TransactionID", how="left"),
        evidence,
    )
...
def main() -> None:
    ...
    # Attach graph relationship features
    train_with_rel, validation_with_rel, evidence = attach_graph_rel_features(train_data, validation_data)

    # Compute categorical mappings from training data only
    cat_mappings = training_category_mappings(train_with_rel)
    # Encode categoricals in both train and validation
    train_encoded = encode_categoricals(train_with_rel, cat_mappings)
    validation_encoded = encode_categoricals(validation_with_rel, cat_mappings)
    ...
    # Graph relationship feature importance
    rel_importance = [row for row in importance if row["feature"] in GRAPH_REL_FEATURES]
    ...
    results = {
        ...
        "graph_rel_feature_importance": rel_importance,
        "evidence": evidence,
    }
    ...
    # Print summary
    print("\n=== Model C-Graph Validation Metrics ===")
    for k, v in results["metrics"].items():
        if k != "confusion_matrix":
            print(f"{k}: {v}")
    print("Confusion Matrix:")
    print(results["metrics"]["confusion_matrix"])
    print("\nTop Graph Relationship Features by Gain:")
    for row in sorted(rel_importance, key=lambda x: -x["importance_gain"]):
...
if __name__ == "__main__":
    try:
        ...
        print(f"train_model_c_graph.py: {error}", file=sys.stderr)
        sys.exit(1)