#!/usr/bin/env python3
"""Train Model D-card: Model B plus five UID features (card1-6 only)."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/models/model_d_card"))
    parser.add_argument("--report", type=Path, default=Path("docs/MODEL_D_CARD_BASELINE.md"))
...
def attach_uid_features(train: pl.DataFrame, validation: pl.DataFrame) -> tuple[pl.DataFrame, pl.DataFrame, dict[str, Any]]:
    ...
    # Select the columns needed for UID_CARD: TransactionID, TransactionDT, card1-6, TransactionAmt
...
def main() -> None:
    ...
    # Print summary
    print("\n=== Model D-card Validation Metrics ===")
...
if __name__ == "__main__":
    try:
        ...
        print(f"train_model_d_card.py: {error}", file=sys.stderr)
        sys.exit(1)