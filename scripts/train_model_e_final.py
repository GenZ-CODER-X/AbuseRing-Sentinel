#!/usr/bin/env python3
"""Train Model E: Model B plus nine entity-level temporal features (card1-6 + addr1)."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("artifacts/models/model_e_final"))
    parser.add_argument("--report", type=Path, default=Path("docs/MODEL_E_FINAL_BASELINE.md"))
...
def attach_entity_features(train: pl.DataFrame, validation: pl.DataFrame) -> tuple[pl.DataFrame, pl.DataFrame, dict[str, Any]]:
    """Build state over train then validation events; labels never enter this function."""
    # Select the columns needed for entity features: TransactionID, TransactionDT, card1-6, addr1, TransactionAmt, ProductCD
    entity_train = train.select(*ENTITY_INPUT_COLUMNS)
    entity_validation = validation.select(*ENTITY_INPUT_COLUMNS)
    entity_events = pl.concat([entity_train, entity_validation], how="vertical")
    entity_features = build_entity_features(prepare_entity_events(entity_events))
    evidence = entity_feature_specification() | {
        "entity_event_row_count": entity_events.height,
        "train_entity_event_row_count": entity_train.height,
        "validation_entity_event_row_count": entity_validation.height,
        "label_column_passed_to_graph_builder": False,
        "same_timestamp_batching": True,
    }
    return (
        train.join(entity_features, on="TransactionID", how="left"),
        validation.join(entity_features, on="TransactionID", how="left"),
        evidence,
    )
...
def main() -> None:
    ...
    # Attach entity features
    train_with_entity, validation_with_entity, evidence = attach_entity_features(train_data, validation_data)

    # Compute categorical mappings from training data only
    cat_mappings = training_category_mappings(train_with_entity)
    # Encode categoricals in both train and validation
    train_encoded = encode_categoricals(train_with_entity, cat_mappings)
    validation_encoded = encode_categoricals(validation_with_entity, cat_mappings)
    ...
    # Entity feature importance
    entity_importance = [row for row in importance if row["feature"] in ENTITY_FEATURES]
    ...
    results = {
        ...
        "entity_feature_importance": entity_importance,
        "evidence": evidence,
    }
    ...
    config_metadata = {
        "experiment": EXPERIMENT,
        "added_entity_features": list(ENTITY_FEATURES),
        "base_experiment": "model_b",
        "entity_feature_specification": entity_feature_specification(),
        "train_boundary": {"min": train_boundary.start, "max": train_boundary.end},
        "validation_boundary": {"min": val_boundary.start, "max": val_boundary.end},
        "random_seed": 42,
    }
    write_json(args.output_dir / "config_metadata.json", config_metadata)
    ...
    reproducibility = {
        ...
        "fixed_training_iterations": 1000,
        "early_stopping_rounds": 50,
        "no_random_split_or_shuffle": True,
        "no_row_or_feature_subsampling": True,
    }
    write_json(args.output_dir / "reproducibility.json", reproducibility)

    # Write feature importance CSV
    write_feature_importance(args.output_dir / "feature_importance.csv", importance)

    # Write entity feature importance JSON
    write_json(args.output_dir / "entity_feature_importance.json", entity_importance)

    # Print summary
    print("\n=== Model E Final Validation Metrics ===")
    for k, v in results["metrics"].items():
        if k != "confusion_matrix":
            print(f"{k}: {v}")
    print("Confusion Matrix:")
    print(results["metrics"]["confusion_matrix"])
    print("\nTop 5 Entity Features by Gain:")
    for row in sorted(entity_importance, key=lambda x: -x["importance_gain"])[:5]:
...
if __name__ == "__main__":
    try:
        ...
        print(f"train_model_e_final.py: {error}", file=sys.stderr)
        sys.exit(1)