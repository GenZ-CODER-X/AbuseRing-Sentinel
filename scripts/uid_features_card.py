"""Label-free, strict-prior event-time temporal UID feature construction for card-only UID.

Computes five temporal aggregates over the UID_CARD field (card1-6 only):
- tb_uid_prior_count
- tb_uid_amt_mean
- tb_uid_amt_std
- tb_uid_recency
...
def build_uid_features(events: pl.DataFrame) -> pl.DataFrame:
    ...
    """
    required = {"TransactionID", "TransactionDT", "card1", "card2", "card3", "card4", "card5", "card6", "TransactionAmt"}
    ...
    for batch in ordered.partition_by("TransactionDT", maintain_order=True):
        ...
        for row in batch_rows:
            # Compute UID_CARD: only if all card1-6 are not null
            c1 = row["card1"]
            c2 = row["card2"]
            c3 = row["card3"]
            c4 = row["card4"]
...
def uid_feature_specification() -> dict[str, Any]:
    ...
    return {
        "graph_entity_nodes": ["card1", "card2", "card3", "card4", "card5", "card6"],
