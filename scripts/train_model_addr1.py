...
MODEL_B_FEATURES = model_a.FEATURES + ADDED_RAW_CATEGORICAL_FEATURES
FEATURES = MODEL_B_FEATURES + ADDR_FEATURES
...
IDENTITY_FEATURES = ("DeviceInfo", "DeviceType")
...
def scan_selected_columns(transaction_path: Path, identity_path: Path) -> pl.LazyFrame:
    ...
    # Features that come from transaction CSV (exclude identity features)
    transaction_columns = [f for f in MODEL_B_FEATURES if f not in IDENTITY_FEATURES]
    transactions = pl.scan_csv(transaction_path).select("TransactionID", *transaction_columns, LABEL_FIELD)
    identities = pl.scan_csv(identity_path).select("TransactionID", *IDENTITY_FEATURES)
    frame = transactions.join(identities, on="TransactionID", how="left")
    # Ensure we have all MODEL_B_FEATURES plus label
    available = set(frame.collect_schema().names())
    required = set(MODEL_B_FEATURES) | {LABEL_FIELD}
...
def write_report(
    ...
    path.write_text(
        ...
        f"| F1 at selected threshold | {compare['f1_at_selected_threshold']['model_b']:.12f} | {compare['f1_at_selected_threshold']['model_addr1']:.12f} | {compare['f1_at_selected_threshold']['difference_addr1_minus_b']:+.12f} |\n\n"
        f"## Model Addr1 operating point\n\n"
        f"Threshold `{model_addr1_metrics['threshold_selection']['selected_threshold']:.12f}` maximizes validation recall under FPR ≤1%, with ties resolved by higher threshold. It yields precision {model_addr1_metrics['precision_at_selected_threshold']:.12f}, recall {model_addr1_metrics['recat_selection']['selected_threshold']:.12f}, F1 {model_addr1_metrics['f1_at_selected_threshold']:.12f}, and FPR {model_addr1_metrics['threshold_selection']['selected_threshold_fpr']:.12f}. Recall at FPR ≤0.1% is {model_addr1_metrics['recall_at_fpr_le_point_1_percent']['recall']:.12f}.\n\n"
...
def main() -> None:
    ...
    # Scan and join for Model B features
